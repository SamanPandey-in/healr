# Day 3 Manual Test Document (Gemini Migration)

## Pre-requisites
- AWS CLI configured with credentials: `aws configure`
- CDK bootstrap completed: `cdk bootstrap`
- Stack deployed: `cdk deploy` (Day 3 version, with `GEMINI_API_KEY` exported — see docs/gemini-migartion.md §5)
- Day 1 and Day 2 tests passed (see TEST_DAY1.md, TEST_DAY2.md)
- **Gemini API key** obtained from [Google AI Studio](https://aistudio.google.com/apikey) and exported:
  ```bash
...
|----------|-------------|
| `SelfHealingInfraStack-DiagnoseWithBedrockFunction*` | Step Functions task #4 — Gemini diagnosis |
...
Additional checks:
- `DiagnoseWithBedrockFunction` → Configuration → Environment variables → confirm `GEMINI_API_KEY` and `GEMINI_MODEL_ID` are set (NOT `BEDROCK_MODEL_ID`)
...
If this step fails with `DIAGNOSIS_NOT_GROUNDED`, the model cited zero valid evidence ids — check `GEMINI_API_KEY` is valid and review CloudWatch Logs for `DiagnoseWithBedrockFunction` for Gemini API errors (e.g. `Gemini API error 400/403`).

If this step fails with a `Gemini API error`, common causes:
- Missing or invalid `GEMINI_API_KEY` — re-export and redeploy
- Model ID mismatch — default is `gemini-2.5-flash`, override with `GEMINI_MODEL_ID` env var
- Rate limiting — retry after a short wait
...
3. The `gemini.converseText` subsegment from `DiagnoseWithBedrockFunction` should appear (replaces the former Bedrock subsegment)
...
## 22. Rollback to Bedrock (If Needed)

If Gemini diagnosis is not working and you need to revert instantly:
...
cd infra && export GEMINI_API_KEY="dummy" && npx cdk deploy
```

Or follow the full rollback steps in docs/gemini-migartion.md §7.

---

## 23. Summary Checklist

- [x] `GEMINI_API_KEY` exported and valid (test with `curl` to Gemini API if unsure)
- [x] 5 new Lambda functions created and configured
- [x] `DiagnoseWithBedrockFunction` has `GEMINI_API_KEY` and `GEMINI_MODEL_ID` env vars (NOT `BEDROCK_MODEL_ID`)
- [x] `InventoryFunction` has a `live` alias with at least 2 published versions
- [x] Step Functions state machine `IncidentResponseDay3` exists with 8 states
- [x] CloudWatch alarm `InventoryErrorAlarm` created
- [x] Healthy requests work with fault injection OFF
- [x] "Good" version (N) published with `INJECT_FAULT=false`
- [x] "Bad" version (N+1) published with `INJECT_FAULT=true`, alias moved to it
- [x] Alarm fires after fault-injected requests
- [x] Execution reaches `RequestApproval` and PAUSES
- [x] `INCIDENT_APPROVAL_REQUIRED` log line with `approveLink` visible in CW Logs
- [x] Approve link returns "Approved" HTML
- [x] Execution resumes and reaches `SUCCEEDED`
- [x] DynamoDB has all 6 rows: META (closed), LOCALIZATION, DIAGNOSIS, APPROVAL (approved), REMEDIATION, VERIFICATION
- [x] `InventoryFunction` alias `live` rolled back to version N
- [x] X-Ray trace shows single causal trace across all Lambdas including `gemini.converseText` subsegment
- [x] Alias rolled back to previous (good) version

## Definition of Done (matches ROADMAP.md Day 3 demo checkpoint)

Inject fault → alarm fires → graph builds → localization ranks correctly → Gemini diagnosis with citations → approval link → click approve → remediation fires → metrics recover → incident closes. The `InventoryFunction` alias `live` is back at the pre-fault version, and all six DynamoDB record types confirm a complete, evidence-backed, human-approved, automated remediation loop.
